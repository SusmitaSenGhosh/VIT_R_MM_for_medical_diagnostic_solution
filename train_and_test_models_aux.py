...
while epoch<epochs:
    ...
    lossSaveTr.append(model.history.history['loss'])
    print("training loss :", model.history.history['loss'][0])
    lossSaveTs.append(model.evaluate(testS,to_categorical(labelTs)))

    pLabel=np.argmax(model.predict(trainS,batch_size= batch_size)[2], axis=1)
    ...
    pLabel=np.argmax(model.predict(testS,batch_size= batch_size)[2], axis=1)
